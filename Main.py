...
with open("casos/" + arquivo) as f:
    ...
    for i in range(0, len(coordenadasPontos)):
        ...
        if(resultado != 0):
            print(str(i+1) + ": " + str(coordenadasPontos[i]) + ", " + str(coordenadasPontos[proximo]) + " = " + str(resultado))
        else:
            doisAFrente = (i+2)%9
            mapaCopia = copy.deepcopy(mapaOriginal)
            resultado = viajar(coordenadasPontos[i], coordenadasPontos[doisAFrente], mapaCopia)
            print(str(i+1) + ": " + str(coordenadasPontos[i]) + ", " + str(coordenadasPontos[doisAFrente]) + " = " + str(resultado))
